...
from test import evaluation_noseg
...
def train(_class_, pars):
    ...
    train_path = '/home/eachgood/Documents/datasets/OCT2017'
    ...
    train_dataloader = torch.utils.data.DataLoader(train_data, batch_size=batch_size, num_workers=8, shuffle=True)
    ...
    with trange(pars.epochs, desc=f'{_class_} training', dynamic_ncols=True, file=sys.stdout) as t:
        for epoch in t:
            lns_layer.train()
            bottleneck.train()
            decoder.train()
            ...
            for img, label, _ in tqdm(train_dataloader, desc=f'Epoch {epoch}', dynamic_ncols=True, leave=False, miniters=50):
                img = img.to(device)
            ...
            auroc, f1, acc = evaluation_noseg(encoder, lns_layer, bottleneck, decoder, test_dataloader, device)
            ...
            t.set_postfix_str(
                f'Loss: {loss / len(train_dataloader):.4f}, S-AUROC: {auroc_sp_best:.4f}, F1:{f1_:.4f}, ACC: {acc_:.4f}')
        # visualize_noseg(model, test_dataloader, device, _class_=_class_, save_name=args.save_name)
    return auroc_sp_best, f1_, acc_
...
if __name__ == '__main__':
    ...
    exp_name = f'OCT2017_reason_gbs_{pars.block_ratio}_{pars.noise_std}_random_shape'
    ...
    for _class_ in all_classes:
        ...
        auroc_sp, f1, acc = train(_class_, parser.parse_args())
        print('class: {}, Auroc sample: {:.4f}, F1:{:.4f}, ACC: {:.4f}'.format(_class_,
                                                                               auroc_sp, f1, acc))

        metrics['class'].append(_class_)
        metrics['AUROC_sample'].append(round(auroc_sp * 100, 2))
        metrics['AUROC_pixel'].append(round(f1 * 100, 2))
        metrics['AUPRO_pixel'].append(round(acc * 100, 2))
